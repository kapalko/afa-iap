"""
This script provides more flexible and customized specification for evaluation.
Notes:
    - Add custom model
    - Add custom environment
    - Set default --out to --checkpoint directory
    - [TODO] --monitor not work with unity-env; seems like unity gives depth map but hxwx3 frame is required
    - [TODO] ray_results still exists --> Trainable class automatically save config
    - [Hacky] Remove line "config["create_env_on_driver"] = True"
Ref: https://github.com/ray-project/ray/blob/master/rllib/rollout.py
"""
import argparse
import collections
import copy
...
EXAMPLE_USAGE = """
python rollout.py D:\\Results\\test\\xxx --run PPO --episodes 1 --monitor
"""
...
def create_parser(parser_creator=None):
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="Number of workers; if not set, will use num_workers in config.")
    parser.add_argument(
        "--num-gpus",
        default=-1,
        type=int,
        help="Number of GPUs; if not set, will use num_gpus in config.")
    return parser
...
def run(args, parser):
    ...
    if "no_graphics" in config["env_config"].keys():
        config["env_config"]["no_graphics"] = args.no_render

    # Register custom model
    register_custom_model(config["model"])

    # Register custom environment
    register_custom_env(config["env"], config["env_config"])
    ...
    if args.out is None:
        args.out = os.path.join(os.path.dirname(args.checkpoint), "rollout")
